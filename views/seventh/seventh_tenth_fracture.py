...
def display_page7_10():
    ...
    with col2:
        st.markdown('<h1 class="center-text">골반 골절</h1>', unsafe_allow_html=True)
    ...
    with open("assets/golban.jpg", "rb") as f:
        img_bytes = f.read()
        img_base64 = base64.b64encode(img_bytes).decode()

    st.markdown(f"""
        <div class="center-img-container">
            <img src="data:image/png;base64,{img_base64}" alt="골반 이미지">
        </div>
    """, unsafe_allow_html=True)

    col1, col2, col3 = st.columns([0.35, 1, 0.35])
    with col2:
        st.markdown('<div class="center-text">골반에 골절이 예상되는 부위를 선택해주세요.</div>', unsafe_allow_html=True)
    symptom_options = [
        "두덩결합",
        "두덩뼈",
        "궁둥뼈",
    ...
    # 버튼을 눌렀을 때 입력된 내용 처리
    if submit:
        st.write(f"증상 부위: 골반-{selected_symptom}")
