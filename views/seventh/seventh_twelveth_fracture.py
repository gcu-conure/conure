...
def display_page7_12():
    ...
    with col2:
        st.markdown('<h1 class="center-text">발 골절</h1>', unsafe_allow_html=True)
    ...
    with open("assets/foot.png", "rb") as f:
        img_bytes = f.read()
        img_base64 = base64.b64encode(img_bytes).decode()

    st.markdown(f"""
        <div class="center-img-container">
            <img src="data:image/png;base64,{img_base64}" alt="발뼈 이미지">
        </div>
    """, unsafe_allow_html=True)

    col1, col2, col3 = st.columns([0.35, 1, 0.35])
    with col2:
        st.markdown('<div class="center-text">발 또는 발목에 골절이 예상되는 부위를 선택해주세요.</div>', unsafe_allow_html=True)
    symptom_options = [
        "목말뼈",
        "발배뼈",
        "쐐기뼈",
    ...
    if submit:
        ...
        #     model="gpt-4",
        # )
        st.write(f"증상 부위: 발-{selected_symptom}")
