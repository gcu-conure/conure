...
def display_page7_9():
    ...
    with col2:
        st.markdown('<h1 class="center-text">손목 및 손 골절</h1>', unsafe_allow_html=True)
    ...
    st.markdown(f"""
        <div class="center-img-container">
            <img src="data:image/png;base64,{img_base64}" alt="손 및 손목뼈 이미지">
        </div>
    """, unsafe_allow_html=True)

    col1, col2, col3 = st.columns([0.35, 1, 0.35])
    with col2:
        st.markdown('<div class="center-text">손에 골절이 예상되는 부위를 그림을 보고 선택해주세요.</div>', unsafe_allow_html=True)
    selected_symptom = st.text_input("부위")
    ...
    if submit:
        ...
        #     model="gpt-4",
        # )
        st.write(f"증상 부위: 손-{selected_symptom}")
