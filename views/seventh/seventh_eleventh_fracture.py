...
def display_page7_11():
    ...
    with col2:
        st.markdown('<h1 class="center-text">다리 골절</h1>', unsafe_allow_html=True)
    ...
    with open("assets/leg.jpg", "rb") as f:
        img_bytes = f.read()
        img_base64 = base64.b64encode(img_bytes).decode()

    st.markdown(f"""
        <div class="center-img-container">
            <img src="data:image/png;base64,{img_base64}" alt="다리뼈 이미지">
        </div>
    """, unsafe_allow_html=True)

    col1, col2, col3 = st.columns([0.35, 1, 0.35])
    with col2:
        st.markdown('<div class="center-text">다리에 골절이 예상되는 부위를 선택해주세요.</div>', unsafe_allow_html=True)
    symptom_options = [
        "대퇴골",
        "슬개골",
        "종아리뼈",
    ...
    if submit:
        ...
        #     model="gpt-4",
        # )
        st.write(f"증상 부위: 다리-{selected_symptom}")
