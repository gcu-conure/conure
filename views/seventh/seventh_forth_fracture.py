...
def display_page7_4():
    ...
    with col2:
        st.markdown('<h1 class="center-text">척추 골절</h1>', unsafe_allow_html=True)
    ...
    with open("assets/spine2.jpg", "rb") as f:
        img_bytes = f.read()
        img_base64 = base64.b64encode(img_bytes).decode()

    st.markdown(f"""
        <div class="center-img-container">
            <img src="data:image/jpeg;base64,{img_base64}" alt="척추 이미지">
        </div>
    """, unsafe_allow_html=True)

    col1, col2, col3 = st.columns([0.35, 1, 0.35])
    with col2:
        st.markdown('<div class="center-text">척추 골절이 예상되는 부위를 선택해주세요.</div>', unsafe_allow_html=True)
    symptom_options = [
        "경추(목뼈)",
        "흉추(등뼈)",
        "요추(허리뼈)",
    ...
    if submit:
        ...
        #     model="gpt-4",
        # )
        st.write(f"증상 부위: 척추-{selected_symptom}")
