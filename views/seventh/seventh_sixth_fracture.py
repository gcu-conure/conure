...
def display_page7_6():
    ...
    with col2:
        st.markdown('<h1 class="center-text">쇄골 골절</h1>', unsafe_allow_html=True)
    ...
    st.markdown(f"""
        <div class="center-img-container">
            <img src="data:image/png;base64,{img_base64}" alt="쇄골 이미지">
        </div>
    """, unsafe_allow_html=True)

    col1, col2, col3 = st.columns([0.35, 1, 0.35])
    with col2:
        st.markdown('<div class="center-text">쇄골 부근에 골절이 예상되는 부위를 선택해주세요.</div>', unsafe_allow_html=True)
    symptom_options = [
        "쇄골",
        "견봉쇄골 관절",
        "견봉",
    ...
    if submit:
        ...
        #     model="gpt-4",
        # )
        st.write(f"증상 부위: 쇄골 부근-{selected_symptom}")
